...
class TSManager:
    ...
    def __init__(self,
        ...
        """ 
        Create and record transforms for every non-auxilliary features in
        the DataFrame.
        
        Auxilliary features are expected to be the right-most columns.
        """
        
        self.transforms_ = dict() 
        for col in list(df.iloc[:, :-num_aux].columns):
            self.transforms_[col] = TSTransformer(df[col].values)
        
    def transmogrify(self, df):
        """ Apply transforms to all non-auxilliary columns of the given DataFrame."""
        
        for key in self.transforms_.keys():
            df.loc[:, key] = self.transforms_[key].transform(df.loc[:, key].values)
        return df
    
    def invertify(self, df):
        """ Invert transforms of all non-auxilliary columns of the given DataFrame."""
        for key in self.transforms_.keys():
...
class TSDS(Dataset):
    ...
    time series and optionally auxilliary features.
    
    Rows are expected to be time steps. Columns are expected to be times series
    or auxilliary features.
    
    Auxilliary features are temporal features that apply to all series, such as
    time step or feature engineered periodicity data. They are NOT static covariates
    of any specific series.
    
    Auxilliary features are assumed to be the right-most columns. They are included in
    ...
    def __getitem__(self, index):
        """ Return an X, y tuple. Auxilliary features are in X, but not y. """
...
def get_tsds(df, lookback, forecast, train_pct, num_aux=0):
    """ Return two TSDS classes for training and testing. """
    
    train_len = round(df.shape[0] * train_pct)
